...
if (Test-Path $portManagerPath) {
    . $portManagerPath
    Write-ToolLog -Message "Port management system loaded"
} else {
    Write-ToolLog -Message "WARNING: Port-Manager.ps1 not found. Using default ports."
}
...
if (-not $OPENAI -or -not $ANTHROPIC -or -not $GEMINI) {
    ...
    if (Test-Path $envFile) {
        Write-ToolLog -Message "Reading API keys from .env file..."
...
if (-not $OPENAI -or -not $ANTHROPIC -or -not $GEMINI) {
    ...
    Write-Output "   OPENAI_API_KEY=your_key"
    Write-Output "   ANTHROPIC_API_KEY=your_key"
    Write-Output "   GEMINI_API_KEY=your_key"
...
$DOCKER_PASSWORD = $env:DOCKER_PASSWORD
  ...
  if ($DOCKER_USERNAME -and $DOCKER_PASSWORD) {
      Write-ToolLog -Message "Logging into Docker Hub..."
      Write-Output $DOCKER_PASSWORD | docker login -u $DOCKER_USERNAME --password-stdin
 }
# ---------------------------
# Install Chocolatey
# ---------------------------
Write-ToolLog -Message "Installing Chocolatey..."
...
# ---------------------------
Write-ToolLog -Message "Installing Node.js, Python, Git, Docker Desktop, VS Code..."
...
Write-ToolLog -Message "Verifying Docker daemon is ready..."
$maxRetries = 10
$retryCount = 0
...
if ($retryCount -eq $maxRetries) {
    Write-ToolLog -Message "WARNING: Docker daemon may not be fully ready, continuing anyway..."
}

# ---------------------------
# Install Void (Open-source Cursor alternative)
# ---------------------------
Write-ToolLog -Message "Installing Void (open-source Cursor alternative)..."
try {
    if (-not (Test-Path "$env:USERPROFILE\void")) {
        ...
        if (Test-Path "$env:USERPROFILE\void") {
            Set-Location "$env:USERPROFILE\void"
...
# ---------------------------
Write-ToolLog -Message "Setting up TabbyML (self-hosted AI coding assistant)..."
Write-ToolLog -Message "TabbyML will be configured in docker-compose.yml"
Write-ToolLog -Message "TabbyML port will be assigned from port management system (11000-12000 range)"
Write-ToolLog -Message "TabbyML can be integrated with Void and other editors"

# ---------------------------
# VS Code Extensions
# ---------------------------
Write-ToolLog -Message "Installing VS Code extensions..."
...
# Cursor IDE
# ---------------------------
Write-ToolLog -Message "Downloading Cursor IDE..."
...
# CLI Tools
# ---------------------------
Write-ToolLog -Message "Installing CLI tools..."
try {
    Write-ToolLog -Message "Installing Python CLI tools..."
    pip install shell-gpt --quiet

    Write-ToolLog -Message "Installing gpt-cli..."
...
# ---------------------------
Write-ToolLog -Message "Setting up Docker Compose environment..."
...
# Port Assignment
# ---------------------------
Write-ToolLog -Message "Assigning ports using port management system..."
...
if (-not $aiToolkitPort) {
    Write-ToolLog -Message "WARNING: Could not find available port for AI Toolkit, using default 11000"
    $aiToolkitPort = 11000
...
if (-not $tabbyMLPort) {
    Write-ToolLog -Message "WARNING: Could not find available port for TabbyML, using default 11080"
    $tabbyMLPort = 11080
    Register-Port -Port $tabbyMLPort -ApplicationName "TabbyML" -Description "TabbyML AI coding assistant"
}

Write-ToolLog -Message "Port assignments:"
Write-ToolLog -Message "  AI Toolkit: $aiToolkitPort"
Write-ToolLog -Message "  TabbyML: $tabbyMLPort"
...
Write-ToolLog -Message "=== Usage Instructions ==="
Write-ToolLog -Message "Run AI commands in the container:"
Write-ToolLog -Message "  docker exec -it ai-toolkit sgpt 'your prompt'"
Write-ToolLog -Message "  docker exec -it ai-toolkit python -c 'import openai; print(openai.__version__)'"
Write-ToolLog -Message ""
Write-ToolLog -Message "Enter interactive shell:"
Write-ToolLog -Message "  docker exec -it ai-toolkit bash"
Write-ToolLog -Message ""
Write-ToolLog -Message "Access your workspace:"
Write-ToolLog -Message "  Files in C:\Users\$env:USERNAME\AI-Tools\workspace are mounted to /app/workspace"

# ---------------------------
# Auto-Launch Tools
# ---------------------------
Write-ToolLog -Message "Launching AI Tools..."
...
if (Test-Path $cursorPath) {
    ...
    Write-ToolLog -Message "WARNING: Skipping Cursor launch (not installed at $cursorPath)"
}
