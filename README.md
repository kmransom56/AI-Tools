...
Docker's `cagent` is included as an optional **inlined** service in `docker-compose.yml` (it is not enabled by default). To enable it locally, create a `cagent/config` directory if you need to mount custom configuration and then run:
```bash
