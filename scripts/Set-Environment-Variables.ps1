...
Write-Host "This script will set API keys as User-level environment variables." -ForegroundColor Yellow
Write-Host "User-level variables apply only to your account (no admin required).`n" -ForegroundColor Yellow
...
$existingGemini = [System.Environment]::GetEnvironmentVariable('GEMINI_API_KEY', 'User')
...
if (Test-Path $envFile) {
    ...
    if ($openaiKey -and $anthropicKey -and $geminiKey) {
        ...
        if ($useEnv -eq 'Y' -or $useEnv -eq 'y') {
            ...
            Write-Host "  `$env:OPENAI_API_KEY = [System.Environment]::GetEnvironmentVariable('OPENAI_API_KEY', 'User')" -ForegroundColor Gray
            Write-Host "  `$env:ANTHROPIC_API_KEY = [System.Environment]::GetEnvironmentVariable('ANTHROPIC_API_KEY', 'User')" -ForegroundColor Gray
            Write-Host "  `$env:GEMINI_API_KEY = [System.Environment]::GetEnvironmentVariable('GEMINI_API_KEY', 'User')" -ForegroundColor Gray
            exit 0
        }
    }
}
